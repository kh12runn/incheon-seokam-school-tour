export async function verifyMonthQuiz(page,baseURL='http://127.0.0.1:8085',{touch=false,denyLock=true}={}){
  ...
  if(!touch){
    for(const [id,y] of [['office-realistic',-1.9],['office-cute',-3.35]]){
      await move({x:40,y:1.5,z:3.4});await page.waitForTimeout(150);
      await page.evaluate(y=>{schoolTour.test.setPosition({x:32.2,y,z:3.4});schoolTour.test.setYaw(Math.PI/2);},y);
      await bubble.waitFor({state:'visible',timeout:60000});
      assert((await page.evaluate(()=>schoolTour.getMonthQuizState())).question.npcId===id,'Office NPC bubble');
      await page.waitForFunction(()=>schoolTour.getOfficePrincipalStates().every(n=>n.modelStatus==='ready'));
    }
  }
