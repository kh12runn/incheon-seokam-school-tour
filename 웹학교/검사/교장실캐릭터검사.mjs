...
assert.equal(OFFICE_CHARACTERS.length,2);
assert.deepEqual(OFFICE_CHARACTERS.map(c=>c.id),['realistic','cute']);
for(const config of OFFICE_CHARACTERS){
  const p=config.position;
  assert.equal(config.height,1.84);
...
const [a,b]=OFFICE_CHARACTERS.map(c=>c.position);
assert(Math.hypot(a.x-b.x,a.y-b.y)>1.2,'두 모델 간격');
...
const normalized=normalizePrincipalModel(asset,1.84),bounds=new THREE.Box3().setFromObject(normalized),size=bounds.getSize(new THREE.Vector3());
const near=(a,b)=>assert(Math.abs(a-b)<1e-6,a+' ≈ '+b);
near(size.y,1.84);near(bounds.min.y,0);near(bounds.min.x+bounds.max.x,0);near(bounds.min.z+bounds.max.z,0);
