...
PC에서는 마우스 드래그뿐 아니라 `W`(위), `S`(아래), `A`(왼쪽), `D`(오른쪽) 키를 누르고 있어도 시점이 부드럽게 움직입니다. 실제 앞뒤 이동은 화면 속 이동 화살표로 다음 촬영 지점에 이동하는 방식입니다.
...
현재 포함된 사진과 로고는 기능 확인용 AI 생성 샘플이며 실제 인천석암초등학교의 모습이나 공식 상징이 아닙니다.

샘플 화면에는 실사 원본 대신 가우시안 스플래팅을 연상시키는 가벼운 3D 게임 렌더링 변환본을 사용합니다. 원본 샘플은 비교와 재작업을 위해 그대로 보존되어 있습니다. 실제 사진을 받을 때의 처리 방법은 `docs/PHOTO_TO_3D.md`를 참고하세요.
