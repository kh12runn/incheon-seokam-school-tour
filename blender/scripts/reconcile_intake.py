...
for s in data['spaces']:
    ...
    if s['building']=='ANNEX':
        ...
        if code in annex:
            ...
            s['photoFolder']=f'photos/raw/{floor}/{category}/{code if category=="classrooms" else code.lower()}/'
...
if not any(s['id']=='2F_GRADE3_RESEARCH' for s in corrected):
    corrected.append({'id':'2F_GRADE3_RESEARCH','name':'3학년 연수실','type':'special_room','floor':'2F','building':'MAIN','blenderObject':'SPECIAL_2F_GRADE3_RESEARCH','photoFolder':'photos/raw/2F/special_rooms/grade3_research/','displayMode':'hybrid','dimensionsStatus':'estimated','layoutStatus':'observed'})
