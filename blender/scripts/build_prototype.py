...
CFG = json.loads((ROOT / 'data/settings.json').read_text(encoding='utf-8'))
D = {k: v['value'] for k,v in CFG['dimensions'].items()}
stamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'blender/backups'/f'before_prototype_{stamp}.blend'), copy=True)
...
spaces=json.loads((ROOT/'data/spaces.json').read_text(encoding='utf-8'))
...
(ROOT/'data/spaces.json').write_text(json.dumps(spaces,ensure_ascii=False,indent=2),encoding='utf-8')
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'blender/school_master.blend'))
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'blender/backups'/f'school_phase04_{stamp}.blend'),copy=True)
print(json.dumps({'scene':scene.name,'objects':len(scene.objects),'rooms':len(room_positions),'file':str(ROOT/'blender/school_master.blend'),'dimensions':'estimated'},ensure_ascii=False))
