...
backup=ROOT/'모델/백업'/('학교모델_'+stamp);backup.mkdir(parents=True)
...
for filename in ['spaces.json','photos.json','settings.json']:
 shutil.copy2(ROOT/'공간자료'/filename,backup/filename)
settings=json.loads((ROOT/'공간자료/settings.json').read_text(encoding='utf-8'))
...
(ROOT/'공간자료/settings.json').write_text(json.dumps(settings,ensure_ascii=False,indent=2),encoding='utf-8')
...
data=json.loads((ROOT/'공간자료/spaces.json').read_text(encoding='utf-8'));byid={s['id']:s for s in data['spaces']}
...
def ensure(sid,name,f,kind='special_room',building='MAIN'):
 if sid not in byid:
  byid[sid]={'id':sid,'name':name,'floor':f'{f}F','type':kind,'building':building,'photoFolder':f'사진보관/원본/{f}F/{kind}s/{sid}/','displayMode':'hybrid','dimensionsStatus':'estimated','layoutStatus':'architectural_plan_matched'}
 return byid[sid]
...
def room(sid,f,x0,x1,y0,y1,door='N',name=None):
 ...
 photo_points.append({'photoPointId':point.name,'spaceId':sid,'rawFolder':s['photoFolder'],'processedFolder':s['photoFolder'].replace('/원본/','/변환본/'),'intakeFolder':s.get('intakeFolder'),'panorama':None,'position':None,'prototypePosition':list(point.location),'positionStatus':'estimated'})
...
for s in byid.values():
 ...
 for p in [s['photoFolder'],s['photoFolder'].replace('/원본/','/변환본/')]: (ROOT/p).mkdir(parents=True,exist_ok=True)
...
(ROOT/'공간자료/spaces.json').write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8')
photos=json.loads((ROOT/'공간자료/photos.json').read_text(encoding='utf-8'));photos['photoPoints']=photo_points
...
(ROOT/'공간자료/campus-check.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
txt=bpy.data.texts.get('build_campus.py') or bpy.data.texts.new('build_campus.py');txt.clear();txt.write((ROOT/'모델/실행도구/build_campus.py').read_text(encoding='utf-8'))
