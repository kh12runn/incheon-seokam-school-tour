...
def photo_folder(floor: str, category: str, leaf: str) -> str:
    if floor == "EXTERIOR":
        return f"사진보관/원본/exterior/{leaf}/"
    if floor == "UNKNOWN":
        return f"사진보관/원본/TODO_unresolved/{leaf}/"
    return f"사진보관/원본/{floor}/{category}/{leaf}/"
...
payload = {
    "schemaVersion": 1,
    "school": "인천석암초등학교",
    "source": "참고자료/배치도/2026학년도 교실배치도.pdf",
...
photos = {
    ...
    "unassigned": [
        {
            "file": "사진보관/원본/촬영위치미확인/IMG_20260908_095512_00_001.insp",
            "camera": "Insta360 X6",
...
(ROOT / "사진보관/원본/촬영위치미확인").mkdir(parents=True, exist_ok=True)
(ROOT / "사진보관/변환본/촬영위치미확인").mkdir(parents=True, exist_ok=True)

for path, data in [
    (ROOT / "공간자료/spaces.json", payload),
    (ROOT / "공간자료/photos.json", photos),
    (ROOT / "공간자료/materials.json", materials),
]:
    if path.exists():
